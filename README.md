...
| 阶段 | 计划内容 |
|:---|:---|
| **P4 交互控制完备** | 把 AI 接到 ERA 的交互原语上（选项、`INPUT` 系、命令触发），让 AI 不只产出文字而能推动流程；引用历史消息（需快照文本副本，防上下文压缩后引用悬空）；手动修改 AI 回复（只影响文本上下文，不回滚已写数值） |
| **P5 上下文压缩** | 对话历史超过约 80% 上下文窗口时由副 API 生成摘要，而不是按固定轮数截断；token 用量统计 |
| **P6 加固** | 提示注入防护、错误路径全覆盖、配置页完善、多步撤销 |
---
